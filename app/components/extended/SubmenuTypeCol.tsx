...
const SubmenuTypeCol = ({
  ...
  const handleChangeSubtype = (e: PointerEvent<HTMLDivElement>) => {
    const config = {
      ...
      ...{
        ...(selected === "date" && {
          parse: e.currentTarget.dataset.value as string,
        }),
        ...(selected === "money" && {
          dsc: e.currentTarget.dataset.value as string,
        }),
      },
    }
      ...
      {opened && (
        ...
        >
          {items.map((item, idx) => (
            <div
              key={item.key}
              className={`relative flex items-center pl-0 py-1.5 group-hover:pl-4 group-hover:hover:pl-7 transition-all duration-200 ease-in-out group/submenu font-normal g rounded-sm ${idx % 2 === 0 ? "bg-white" : ""}`}
              data-value={item.key}
              onPointerDown={handleChangeSubtype}
            >
