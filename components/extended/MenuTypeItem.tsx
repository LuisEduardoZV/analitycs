...
const MenuTypeItem = ({ label, icon, hasSubmenu, items, type, selected, columnKey, onChangeType }: MenuTypeItemProps) => {
    return (
        <div className="flex gap-1 items-center flex-row-reverse flex-wrap justify-between p-1 after:contents[''] after:w-4/5 after:border-b-1 after:border-default-200/45 after:absolute relative after:bottom-0 after:left-0 hover:cursor-pointer after:transition-all after:duration-700 after:ease-in-out group hover:after:w-full hover:after:border-b-secondary/40" onPointerDown={(e) => {onChangeType(e, type)}}>
            <div
            ...
            <span className="pl-0 group-hover:pl-3 transition-all duration-200 ease-in-out">{label}</span>
            {hasSubmenu && items && (<SubmenuTypeCol type={type} selected={selected} items={items} columnKey={columnKey} />)}
        </div>
    )
}
